...
# 本地数据优先导入
try:
    from common import local_data

    LOCAL_DATA_AVAILABLE = local_data.is_local_data_available()
except Exception:
...
def fetch_basic_info(symbols: list[str]) -> list[dict]:
    ...
    if LOCAL_DATA_AVAILABLE:
        try:
            local_results = local_data.get_basic_info(symbols)
            if local_results:
                # 转换格式
...
def fetch_financial_metrics(symbol: str) -> dict:
    ...
    if LOCAL_DATA_AVAILABLE:
        try:
            local_metrics = local_data.get_financial_metrics(symbol)
            if local_metrics:
...
def fetch_price_history(symbol: str, period: str = "1y", adjust: str = "qfq") -> dict:
    """
    ...
    if LOCAL_DATA_AVAILABLE:
        try:
            local_prices = local_data.get_price_history(
                symbol, period=period, adjust=adjust, start_date=start, end_date=end
            )
            if local_prices:
                return {
                    "symbol": sym,
                    ...
                    "start_date": local_prices[0]["date"] if local_prices else "",
                    "end_date": local_prices[-1]["date"] if local_prices else "",
                    "prices": local_prices,
                    "source": "local",
                }
        except Exception:
    ...
    try:
        df = ak.stock_zh_a_hist(
            symbol=sym, period="daily", start_date=start, end_date=end, adjust=adjust
        )
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--metrics", action="store_true", help="Full financial metrics")
    parser.add_argument("--history", action="store_true", help="Price history")
    parser.add_argument(
        "--financials", action="store_true", help="Financial statements"
    )
    parser.add_argument("--insider", action="store_true", help="Insider trading data")
    parser.add_argument(
        "--northbound", action="store_true", help="Northbound capital flow"
    )
    parser.add_argument(
        "--screen", action="store_true", help="Screen against default filters"
    )
    parser.add_argument(
        "--period", default="1y", help="History period (1m,3m,6m,1y,2y,5y,max)"
    )
